...
data = np.genfromtxt('log.lammps', skip_header=63, max_rows=101)

# Columns
step = data[:, 0]
temp = data[:, 1]

# Keep only data after timestep 5000
mask = step >= 5000
...
ax.axvline(x=5000, linestyle="--", linewidth=1, label="Equilibration cutoff")
