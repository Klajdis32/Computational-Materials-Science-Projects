...
data = np.genfromtxt('log.lammps', skip_header=63, max_rows=11)

# Columns
step = data[:, 0]
temp = data[:, 1]

# Keep only data after timestep 1000
mask = step >= 1000
...
ax.axvline(x=1000, linestyle="--", linewidth=1, label="Equilibration cutoff")
